...
InfoSystemServicesDependencyMatrix::InfoSystemServicesDependencyMatrix(QWidget* parent) : QDialog(parent)
{
	...
	connect(add, SIGNAL(clicked()),
			this, SLOT(addDependencyRelation()));
	connect(del, SIGNAL(clicked()),
			this, SLOT(delDependencyRelationElement()));


}

void InfoSystemServicesDependencyMatrix::revealISS(const QModelIndex& index)
{
	...
	iSSListQuery->setQuery(QString("SELECT name AS ISS FROM IssDependency INNER JOIN InformationSystemServices ON InformationSystemServices.information_service_id = IssDependency.depends WHERE IssDependency.information_service_id = %1 GROUP BY name").arg(currentISSId));
		
	iSSList->setModel(iSSListQuery);
}

void InfoSystemServicesDependencyMatrix::addDependencyRelation()
{
	...
	if (dlgHelper->exec()==QDialog::Accepted){
		...
		foreach(QString element, currentElementsInTheList){
		qry.prepare("INSERT INTO IssDependency (information_service_id, depends) VALUES (:information_service_id, :depends)");
...
void InfoSystemServicesDependencyMatrix::delDependencyRelationElement()
{
	QSqlQuery qry;
	...
	qry.prepare("DELETE FROM IssDependency WHERE information_service_id = :information_service_id AND depends = :depends");
